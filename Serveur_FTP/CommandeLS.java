...
public class CommandeLS extends Commande {
	...
	public void execute() {
		...
		if(commandeArgs.length==0) {
			file = new File(Commande.CWD);
		}else {
			file = new File(Commande.CWD+FileSystems.getDefault().getSeparator()+commandeArgs[0]);
		}
		if(file.exists()) {
			if(!file.isDirectory()) {
				if(commandeArgs.length != 0) {
					...
					if(file.exists()) {
						if(!file.isDirectory()) {
							ps.println("2 "+file.getAbsolutePath()+" n'est pas un répertoire");
							return;
						}
					}else {
						ps.println("2 "+file.getAbsolutePath()+" n'existe pas");
						return;
					}
			}
		}else {
			...
			return ;
						...
						String entete = null;
						try {
							entete = "\" ------------------------Listing de : \"" +file.getCanonicalPath()+ "\" -------------------------------";
						} catch (IOException e) {
							e.printStackTrace();
						}
						ps.println("1 "+entete);
